...
const requestBody = (options: Options, request: MaterialProviderRequest): unknown => ({
  ...
  instructions:
    "Create exactly three materially different Japanese learning presentations for the one target Card. Use only the supplied supporting vocabulary and grammar. Write the Japanese as a remark someone in the scene the context sets would actually say — a whole remark, not a phrase, and not a dictionary example of the target: give the target the particles and the words it normally goes with, so the sentence shows how the word is used. Keep to standard forms: a spoken contraction such as してる is not a word the learner has, so write している, and every ending and connective must be one that allowedSupportingGrammar lists. Do not pad — a longer sentence is only better if every word in it is still one the learner has. The English context sets the scene in one sentence — who is speaking, where, in what mood — and must not state, paraphrase, translate, or hint at the target's meaning or the action it names; a learner reading the context alone must not be able to guess the target. Where mode is review the rest of the Japanese must not do the target's work either: with the target taken out, more than one word should still fit the gap. Where mode is teach the learner is meeting the word rather than recalling it, so the situation may be set out plainly. answer is what the whole Japanese sentence means, in English; explanation says how it works, naming what the target itself means; usageNote says when to use it. context, answer, explanation, and usageNote are English prose; Japanese appears in them only as the target word or a short quoted form. Copy target identity fields exactly. targetSpan is a zero-based UTF-16 code-unit span in NFKC Japanese. For a grammar target, span the whole target word; the construction's own detected form falls inside that span. Reading segments must reconstruct Japanese exactly. Do not include another learning target.",
...
const batchRequestBody = (
  ...
  return {
    ...
    store: false,
    reasoning: { effort: "low" },
    instructions: `For every target Card in "targets", create exactly ${perCard} materially different Japanese learning presentations. Return one item per target, in the same order, with cardId copied exactly. Use only the supplied supporting vocabulary and grammar, shared by all targets. Write the Japanese as a remark someone in the scene the context sets would actually say — a whole remark, not a phrase, and not a dictionary example of the target: give the target the particles and the words it normally goes with, so the sentence shows how the word is used. Keep to standard forms: a spoken contraction such as してる is not a word the learner has, so write している, and every ending and connective must be one that allowedSupportingGrammar lists. Do not pad — a longer sentence is only better if every word in it is still one the learner has. Where a target lists previousRejections, an earlier attempt was refused for those reasons (for example an unknown word it used); do not repeat them. The English context sets the scene in one sentence — who is speaking, where, in what mood — and must not state, paraphrase, translate, or hint at the target's meaning or the action it names; a learner reading the context alone must not be able to guess the target. Where mode is review the rest of the Japanese must not do the target's work either: with the target taken out, more than one word should still fit the gap. Where mode is teach the learner is meeting the word rather than recalling it, so the situation may be set out plainly. answer is what the whole Japanese sentence means, in English; explanation says how it works, naming what the target itself means; usageNote says when to use it. context, answer, explanation, and usageNote are English prose; Japanese appears in them only as the target word or a short quoted form. Copy target identity fields exactly. targetSpan is a zero-based UTF-16 code-unit span in NFKC Japanese. For a grammar target, span the whole target word; the construction's own detected form falls inside that span. Reading segments must reconstruct Japanese exactly. Do not include another learning target in any sentence.`,
