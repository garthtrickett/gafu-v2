...
The same spelling with meaningfully different senses may require separate
Vocabulary Cards. The exact sense-identity rule remains an open product
decision.
...
creating extra learning work. It can grow as Vocabulary Cards become known, and
the learner must be able to correct it when the baseline does not match what
they actually know. The exact learner-state threshold for automatically adding
a Vocabulary Card remains an open product decision.
...
The learner is ready for an episode when its required preparation Cards have
reached the minimum learning state chosen for V2. The Preparation Plan may show
later episodes as not ready while an earlier episode is ready to watch. The
minimum state and any coverage threshold are still to be decided.
...
1. Does a Vocabulary Card represent a lemma alone, or a lemma plus a particular
   sense?
2. May an exact episode sentence appear during preparation or later review, or
   should it be reserved for natural playback?
3. What minimum state makes a required Card ready for playback: taught once,
   recalled once, or recalled after a delay?
4. How should Gafu estimate that an episode is sufficiently covered?
5. What evidence and threshold decide whether a gap item is required, helpful,
   or incidental?
6. Are Cards private to one learner, shared catalogue entries with private
   progress, or a mixture of both?
7. Is V2 permanently single-user, or must its first data model preserve a path
   to multiple accounts and cross-device synchronization?
8. Which parts of the existing context-first and ear-first study experience
   remain essential?
9. How many validated generated presentations should be prepared ahead of time
   so study can continue through an AI outage?
10. What exact similarity rule prevents a generated sentence from being an
    ineffective near-copy of recent material?
11. At what learner state does a Vocabulary Card enter the Known Word Bank, and
    at what state does a Grammar Card become available as supporting grammar?
12. What archive size, file-count, subtitle encoding, and extraction limits are
    appropriate for a complete series import?
13. Should a new Preparation Plan include all required items by default, or ask
